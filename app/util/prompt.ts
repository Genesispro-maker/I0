...
Linear: bg-gray-950 accent indigo-500 text gray-100 muted gray-500
...
✅ <thinking> content uses markdown internally (##, -, ***, \`code\`)
✅ Wrap file names inside <thinking> with <code></code> e.g. <code>App.tsx</code>
❌ JSON block is NEVER wrapped in markdown fences — never \`\`\` or \`\`\`json
❌ No markdown outside of <thinking> and <suggestions>`
...
STEP 3: <suggestions>...</suggestions>   ← only after JSON } is closed
...
IMPORTS:
  ...
  ✅ import { Button } from "./components/Button"
  ❌ import React from "react"
  ❌ Default imports of named exports → CRASH
...
╚══════════════════════════════════════════════════════════════╝
  ...
  ✅ <thinking> uses markdown internally (##, -, ***, \`code\`)
  ✅ Wrap file names inside <thinking> with <code></code>
  ❌ JSON block is NEVER wrapped in markdown fences — never \`\`\` or \`\`\`json
  ❌ No markdown outside of <thinking> and <suggestions>\``
